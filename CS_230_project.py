...
# import data from small_fic_dataset.tsv 
filename = "./data/small_fic_dataset.tsv"

df = pd.read_csv(filename,delimiter='\t',encoding='latin-1')
df.head()

# here: dropping columns that are not required 
df.drop(['Unnamed: 2', 'Unnamed: 3', 'Unnamed: 4'],axis=1,inplace=True)
df.info()

# visualizing distribution of data here 
sns.countplot(df.MAINTAG)
plt.xlabel('Label')
plt.title('Number of fics in each rating')
plt.show()

# Creating input and output vectors, processing the labels.
X = df.TEXT
Y = df.MAINTAG
le = LabelEncoder()
...
# tokenize the words in the story text here 
max_words = 5000
max_len = 220
